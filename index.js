...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5511970115273:+55 1197011-5273\n' // WhatsApp ID + phone number
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Ola@${num.split('@')[0]}\nBem vindo ao grupo, Mariano mandou vc se comportar para não tomar ban ok *${mdata.subject}*`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Mano(a) 🕐esepre estou processando🕐',
				success: '✔️ Sucesso ✔️',
				...
				error: {
					stick: '[❗] Falha, mané deu erro em converter a imagem em um adesivo ❌',
					Iv: '❌ Link invalido ❌'
				},
				only: {
					group: '[❗] ERROR, O COMANDO PODE SER USADO APENAS EM GRUPOS! ❌',
					ownerG: '[❗] KKK, O COMANDO SÓ PODE SER USADO PELO MARIANO! ❌',
					ownerB: '[❗] KKK, O COMANDO SÓ PODE SER USADO PELO MARIANO! ❌',
					admin: '[❗] KAKAKAKA, ESSA É BOA MEMBRO COMUM TENTANDO USAR COMANDOS DE ADMS KKKKK! ❌',
					Badmin: '[❗] O BOT PRECISA SER ADEMIR! ❌',
			...
			const ownerNumber = ["5511970115273@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5511970115273@s.whatsapp.net'
                        /******ApiKey Input******/
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               client.sendMessage(from, 'Aqui esta o nuemro do meu dono mané, salva depois',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'ytmp3':
					...
					teks = `❏ *Titulo* : ${anu.title}\n❏ *Tamanho* : ${anu.filesize}\n\nEspere um pouco ai mano, Audio Esta Sendo Enviado Novamente...`
	...
	case 'kbbi':
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'chord':
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('oshh cade a letra da musica heim mané?')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'kick':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Vaza daqui filha da putisse removido com sucesso kkkkkk :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Pedido recebido, se manca vaii  : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
				...
				case 'exe':
	              ...
	              if (!isOwner) return reply(mess.only.ownerB)
	               ...
	               exec(cmd, (err, stdout) => {
		           if(err) return client.sendMessage(from, "ERROR, COMANDO INCORRETO", text, { quoted: mek })
		           if (stdout) {
				...
				case 'ocr':
					...
					break
                     ...
                     const bug = body.slice(5)
                      if (pesan.length > 300) return client.sendMessage(from, 'OLOKOOOO, TEXTO GRANDE DA MIZERAAA, ENVIEI UM MENOR (minimo 300 letras)', msgType.text, {quoted: mek})
                    ...
                    reply('O BUG FOI REPORTADO PRO MARIANO, SE NÃO FOR VERDADE NOIS VAI COBRA MANÉ/main2 E NÃO SERA RESPONDIDO.')
                    break
               case 'apakah':
               client.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'closegc':
					...
					var nomor = mek.participant
					const close = {
					text: ` ademiro fechou o grupo seus corno kskskssk @${nomor.split("@s.whatsapp.net")[0]}\nagora *so os pika* das galaxias pode falar `,
					contextInfo: { mentionedJid: [nomor] }
					}
                ...
                case 'bukagc':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					open = {
					text: `Grupo aberto pelo ademiro @${sender.split("@")[0]}\nagora *os membros comum* podem falar `,
					contextInfo: { mentionedJid: [sender] }
					}
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ ERROR, no momento da conversão ${tipe} ke stiker`)
							})
							.on('end', function () {
								console.log('Finish')
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ ERROR ao converter adesivos em imagens ❌')
                ...
                case 'ninjalogo':
                      if (args.length < 1) return reply('KD O TXT HEIMM?')
                      if (!isUser) return reply(mess.only.daftarB)
                      gh = body.slice(11)
                      gl1 = gh.split("|")[0];
                      gl2 = gh.split("|")[1];
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `*Musica Encontrada!!!*\nTitulo : ${anu.result.title}\nFonte : ${anu.result.source}\nTamanho : ${anu.result.size}\n\n*ESPERA AE MANÉ ENVIANDO, NAO ENVIE SPAM *`
                ...
                break
                   ...
                   break
					...
					break
                                  case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('voce ja esta registrado cabeça de rola kkkkkkk')
					if (args.length < 1) return reply(`Parametro Errado\nComando : ${prefix}daftar nome | idade\nExemplos : ${prefix}daftar MARIANO|12`)
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('ativado kraiii ksksks')
						welkom.push(from)
					...
					if (args.length < 1) return reply(mess.blank)
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
					if (tels.length > 15) return reply('O txt é mt grande affss, at� 20 caracteres')
					reply(mess.wait)
            ...
            break
			       ...
			       reply(anu.result.jadwalTV)
					break
                                ...
                                case 'wolfmetal':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}wolfmetal mariano')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'randomkpop':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui kpop ok kpoper bleerr...'})
                                        break
                                case 'fml2':
				...
				case 'simi':
					...
					//if (anu.error) return reply('Simi N�o sei ')
					reply(anu)
					break
				case 'simih':
					if (!isGroup) return reply(mess.only.group)
				...
				case 'clone':
					...
					break
                                ...
                                case 'pubglogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply('onde est� o texto um \ nExemplos: ${prefix}pubglogo Mariano|linduu')
                                        if (!isUser) return reply(mess.only.daftarB)
                                ...
                                case 'harrypotter':
                                        var gh = body.slice(12)
                                        if (args.length < 1) return reply('teksnya mana(o) um\nContoh: ${prefix}harrypotter Mariano ama vcs')
                                        if (!isUser) return reply(mess.only.daftarB)
